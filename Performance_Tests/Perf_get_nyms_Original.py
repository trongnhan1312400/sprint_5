...
# Run this script to process output from the .txt output from perf_add_requests.py or the add_nyms.py file.  This script will
...
async def get_nyms(threadnum):
    ...
    pool_genesis_txn_path = 'qalive_pool'
...
logger.info(f"------ Average time per file in seconds {average_time_per_file:.2f}")
logger.info(f"------ Number of NYMs per second {nyms_per_second:.2f}")
