...
parser = argparse.ArgumentParser(description='This script will create multiple threads of the perf_add_requests.py or '
                                             'the Perf_get_nyms.py.')

parser.add_argument('-a', help='Use this parameter to start perf_add_requests.py', action='store_true',
...
if results.a:
    results.a = 'perf_add_requests.py'
if results.g:
...
def run_test(i, lock):
    ...
    if results.a:
        ...
        command = 'python3.6 ' + results.a + ' -n ' + str(txns) + ' -s ' + repr(i)
    elif results.g:
        # The default values for -d -t and -g in get_nym will be used
        command = 'python3.6 ' + results.g + ' -s ' + repr(clients) + ' -d nym_files'
    else:
