...
For writing a new claim definition in the ledger, a person or an organization
in that ledger must have one of these roles: Trustee, Steward or Trust Anchor.
...
to build a claim_def request. Then submit it to the ledger.
...
* Build and submit claim_def request to the ledger.
  Step 6. Prepare data and build the claim_def request.
  Step 7. Submit the claim_def request and get the response.
          The new claim definition is written to the ledger
'''
...
async def build_claim_def_request():
    try:
        ...
        # 6. Prepare data and build the claim_def_request.
        print_log("6. Build a claim_def request")
        schema_seq_no = 1
        signature_type = "CL"  # only CL supported now.
        data_request = '{"primary":{"n":"1","s":"2","rms":"3",' \
                       '"r":{"name":"1"}, "rctxt":"1","z":"1"}}'
        claim_def = await ledger.build_claim_def_txn(
                                            default_steward_did, schema_seq_no,
                                            signature_type, data_request)
        print_log("DONE - claim_def: " + str(claim_def))

        # 7. Send the claim definition request and get the response.
        print_log("7. Send the claim_def request to the pool ledger")
...
loop.run_until_complete(build_claim_def_request())
